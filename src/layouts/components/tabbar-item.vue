...
const { route, value, closable } = defineProps<{
...
<template>
  <n-el
    ...
    class="mx-2px px-4 py-2 b b-[--divider-color] b-b-[#0000] rounded-[--border-radius] cursor-pointer" style="transition: box-shadow .3s var(--n-bezier), color .3s var(--n-bezier), background-color .3s var(--n-bezier), border-color .3s var(--n-bezier);"
  >
    <div class="flex gap-2 text-nowrap items-center">
      <icon :icon="route.meta.icon" />
      <span>{{ route.meta.title }}</span>
      <button v-if="closable" class="text-[var(--close-icon-color)] rounded-3px bg-transparent flex size-18px items-center justify-center hover:bg-[var(--close-color-hover)]" @click.stop="emit('close', route.fullPath)">
        <n-icon size="14">
